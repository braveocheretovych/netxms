...
EPRule::EPRule(const ConfigEntry& config, bool nxslV5) : m_timeFrames(0, 16, Ownership::True), m_actions(0, 16, Ownership::True)
{
   ...
   if (actionsRoot != nullptr)
   {
      ...
      for(int i = 0; i < actions->size(); i++)
      {
         ...
         bool inactive = actions->get(i)->getSubEntryValueAsBoolean(_T("inactive"), false);
         if (!guid.isNull())
         {
            uint32_t actionId = FindActionByGUID(guid);
            if (actionId != 0)
               m_actions.add(new ActionExecutionConfiguration(actionId, MemCopyString(timerDelay), MemCopyString(snoozeTime), MemCopyString(timerKey), MemCopyString(blockingTimerKey), inactive));
         }
         else
         {
...
EPRule::EPRule(const NXCPMessage& msg) : m_timeFrames(0, 16, Ownership::True), m_actions(0, 16, Ownership::True)
{
   ...
   if (msg.isFieldExist(VID_RULE_ACTIONS))
   {
      ...
      for(int i = 0; i < actions.size(); i++)
      {
         m_actions.add(new ActionExecutionConfiguration(actions.get(i), nullptr, nullptr, nullptr, nullptr, 0));
      }
   }
   else
   {
      ...
      for(int i = 0; i < count; i++)
      {
         ...
         uint32_t flags = msg.getFieldAsUInt32(fieldId++);
         fieldId += 4;
         m_actions.add(new ActionExecutionConfiguration(actionId, timerDelay, snoozeTime, timerKey, blockingTimerKey, flags));
      }
   }
...
bool EPRule::loadFromDB(DB_HANDLE hdb)
{
   ...
   // Load rule's actions
   _sntprintf(szQuery, 256, _T("SELECT action_id,timer_delay,timer_key,blocking_timer_key,snooze_time,inactive FROM policy_action_list WHERE rule_id=%d"), m_id);
   hResult = DBSelect(hdb, szQuery);
   if (hResult != nullptr)
   {
      int count = DBGetNumRows(hResult);
      for(int i = 0; i < count; i++)
      {
         ...
         bool inactive = DBGetFieldUInt32(hResult, i, 5);
         m_actions.add(new ActionExecutionConfiguration(actionId, timerDelay, snoozeTime, timerKey, blockingTimerKey, inactive));
      }
      DBFreeResult(hResult);
   }
   else
...
bool EPRule::saveToDB(DB_HANDLE hdb) const
{
   ...
   {
      DB_STATEMENT hStmt = DBPrepare(hdb, _T("INSERT INTO policy_action_list (rule_id,action_id,timer_delay,timer_key,blocking_timer_key,snooze_time,inactive) VALUES (?,?,?,?,?,?,?)"), m_actions.size() > 1);
      if (hStmt != nullptr)
      {
         DBBind(hStmt, 1, DB_SQLTYPE_INTEGER, m_id);
         for(int i = 0; i < m_actions.size() && success; i++)
         {
            ...
            DBBind(hStmt, 7, DB_SQLTYPE_VARCHAR, a->inactive ? _T("1") : _T("0"), DB_BIND_STATIC);
            success = DBExecute(hStmt);
         }
         DBFreeStatement(hStmt);
      }
...
void EPRule::createMessage(NXCPMessage *msg) const
{
   ...
   for(int i = 0; i < m_actions.size(); i++)
   {
      ...
      msg->setField(fieldId++, a->inactive);
      fieldId += 4;
   }
...
json_t *EPRule::toJson() const
{
   ...
   for(int i = 0; i < m_actions.size(); i++)
   {
      ...
      json_object_set_new(action, "inactive", json_boolean(d->inactive));
